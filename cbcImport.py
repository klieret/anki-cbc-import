...
class cbcImport():
	...
	def insert(self):
		...
		self.e.note['Expression']=current[0].split(delim)[-1]
		self.e.note['Meaning']=current[2]
		# ---------------
		self.e.note.flush()
		self.e.loadNote()
		self.runHooks()
	...
	def cardAdded(self, obj, note):
		current=self.data[self.currentIdx]
		print(note['Expression'],current[0])
	...
	def mySetupButtons(self,editor):
		...
		# --
		self.myAddButton("cbcLoad", self.load, text="Load", tip="Load file", size="30x60", )
		self.myAddButton("cbcReverse", self.reverse, text="Reverse", tip="Reverse Order", size="30x60", )
		self.myAddButton("cbcFirst", self.first, text="<<", tip="Fill in first entry", size="30x30",)
		self.myAddButton("cbcPrevious", self.previous, text="<", tip="Fill in previous entry (Ctrl+H)", size="30x30" , key="Ctrl+H")
		self.myAddButton("cbcFill", self.insert, text="X", tip="Fill in form (Ctrl+F)", size="30x30",  key="Ctrl+F")
		self.myAddButton("cbcNext", self.next, text=">", tip="Fill in next entry (Ctrl+G)", size="30x30", key="Ctrl+G")
		self.myAddButton("cbcLast", self.last, text=">>", tip="Fill in last entry", size="30x30" , )
		self.status=QLabel()
		self.updateStatus()
...
#			b.setFixedWidth(int(size.split('x')[1]))
	...
	def updateStatus(self):
		...
		self.status.setText(" Idx: %d/%d Add: %d Rem: %d" % (self.currentIdx+1,len(self.data),len(self.added),len(self.data)-len(self.added)))	
